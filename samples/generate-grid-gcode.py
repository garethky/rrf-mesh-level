# This script produces the G-Code lines that go into the bed.g macro
# it contains the minimum number of X/Y coordinate definitions to expand into the final probing points
# * X coordinates are reflected about the reflection line (the middle of the bed) by the script
...
# * The order of the rowDefinitions does not matter, they get sorted by the Y coordinate from least to greatest


import collections

# this is the X coordinate of the line reflection, values less than this value are reflected, values exactly at this line or greater are not
reflectionLine = 150

# format is ([X, X, X, X], [Y, Y, Y])
# The group of X coordinates is the left half of a single row of probing points
# Each Y value is the Y coordinate of a complete probing row where all of these X points will be sampled
...
# First extend the X arrays by reflecting them about the reflectionLine
def reflectX():
    for rowDef in rowDefinitions:
        xArray = rowDef[0]
        for xVal in reversed(xArray):
            if (xVal < reflectionLine):
                xArray.append(reflectionLine + (reflectionLine - xVal))

# Then the rowDefinitions are expanded into a map with a unique Y value as the key for each row of probe points
